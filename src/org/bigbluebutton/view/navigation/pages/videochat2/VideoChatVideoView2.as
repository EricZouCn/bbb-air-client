package org.bigbluebutton.view.navigation.pages.videochat2
{
	...
	public class VideoChatVideoView2 extends VideoView
	{
		...
		public function setVideoPosition2(name:String, containerWidth:int, containerHeight:int, leftEdge:int):void
		{
			if (video && stage.contains(video))
			{
				stage.removeChild(video);
			...
			resizeForPortrait();
			var topActionBarHeight : Number = FlexGlobals.topLevelApplication.topActionBar.height;
			video.y = topActionBarHeight + (containerHeight - video.height);
			if(leftEdge < (containerWidth - video.width))
				video.x = leftEdge;
			else
		...
		{
			_loader.x= video.x + (video.width - _loader.content.width)/2;
			_loader.y = video.y + (video.height-_loader.content.height)/2;
			_loader.alpha = 0.5;
